...
def markov(X1):
    X2 = np.dot(X1, X1)
    X2 = np.power(X2, 2)
    X2 = np.dot(np.linalg.inv(np.diag(np.sum(X2, axis=1))), X2)
    X2 = (X2 + X2.T) / 2
...
def train(graph_matrix, true_label, train_data, test_data, kmer_length, result_path, data_info, random_seed, result_dir_path, GPU_option):
    ...
    graph_matrix = np.dot(np.linalg.inv(np.diag(np.sum(graph_matrix, axis = 1))), graph_matrix)
    ...
    with tf.Session(config = config) as sess:
        ...
        for i in range(steps):
            _, total_cross_entropy, total_train_accuracy, total_train_auc = sess.run(
                [train_step, cross_entropy, train_accuracy, train_auc], feed_dict={x: graph_matrix, x_: markov_graph_matrix, z: feature_matrix, y_: true_label})
            if i % 50 == 0:
                ...
                total_test_accuracy, total_test_auc, summary = sess.run([test_accuracy, test_auc, merged],
                                                                        feed_dict={x: graph_matrix, x_: markov_graph_matrix, z: feature_matrix, y_: true_label})
