...
def train(graph_matrix, true_label, train_data, test_data, kmer_length, result_path, data_info, random_seed, result_dir_path, GPU_option):
    ...
    #graph_matrix = np.dot(graph_matrix, graph_matrix)
    graph_matrix = markov(graph_matrix)
    graph_matrix = graph_matrix.astype(np.float32)
    true_label = true_label.astype(np.float32)

    # 开始tensorflow阶段
    #w1 = tf.Variable(tf.random_normal([graph_matrix.shape[0], 1], stddev=1, seed=1))
    w1 = tf.get_variable("w1", shape=[graph_matrix.shape[0], 1], initializer=tf.contrib.layers.xavier_initializer())
