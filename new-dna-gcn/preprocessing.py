...
def weight():
    ...
    for i in range(2, 100):
        weight_vector[i - 2] = np.exp(-np.abs((i + 0.5) - 51) / 2)
...
def transform_one_hot_X_into_kmer_frequences(one_hot_X, kmer_len, slide_window):
    ...
    weight_frequence_array = np.zeros((one_hot_X.shape[0], len(kmers_list)))
    ...
    for i in range(one_hot_X.shape[0]):
        ...
        for start_index in range(len(s) - kmer_len + 1):
            ...
            weight_frequence_array[i, kmer_id_dict[this_kmer]] += weight_vector[start_index]
    return frenquces_array, weight_frequence_array
...
def data_preprocessing(train_dataset, test_dataset, kmer_len, slide_window):
    ...
    train_frequence_array, train_weight_frequence_array = transform_one_hot_X_into_kmer_frequences(train_data, kmer_len, slide_window)
    test_frequence_array, test_weight_frequence_array = transform_one_hot_X_into_kmer_frequences(test_data, kmer_len, slide_window)
    frequence_array = np.vstack((train_frequence_array, test_frequence_array))
    weight_frequence_array = np.vstack((train_weight_frequence_array, test_weight_frequence_array))
    ...
    #A2 = weight_frequence_array * (np.ones(a1).reshape(a1, 1) * np.log(a1 / (1 + count)))
    A3 = np.transpose(A2)
    ...
    print(time.time() - t)
    return A, A_, label, train_frequence_array, test_frequence_array, train_weight_frequence_array, test_weight_frequence_array
